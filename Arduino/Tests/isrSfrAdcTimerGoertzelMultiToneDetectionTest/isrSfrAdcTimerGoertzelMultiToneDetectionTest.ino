...
const uint16_t target_freq_A = 320; // Hz
const uint16_t target_freq_B = 580; // Hz
...
const int16_t gtzl_k_A = 0.5 + ((int32_t)block_size * target_freq_A / sample_rate);
const double gtzl_w_A = (TWO_PI / block_size) * gtzl_k_A;
const double gtzl_cosine_A = cos(gtzl_w_A);
const double gtzl_coeff_A = 2 * gtzl_cosine_A;
const int16_t gtzl_k_B = 0.5 + ((int32_t)block_size * target_freq_B / sample_rate);
const double gtzl_w_B = (TWO_PI / block_size) * gtzl_k_B;
const double gtzl_cosine_B = cos(gtzl_w_B);
const double gtzl_coeff_B = 2 * gtzl_cosine_B;

// variables to store per-sample processing results
volatile double Q0_A, Q1_A, Q2_A, Q0_B, Q1_B, Q2_B;
volatile uint32_t num_adc_conversions;

// variable to store measured magnitude (squared) of target frequencies
double gtzl_magnitude_A, gtzl_magnitude_B;
...
void setup() {
  ...
  Serial.print("Target frequency A: ");
  Serial.print(target_freq_A);
  Serial.println(" Hz");

  Serial.print("k = ");
  Serial.println(gtzl_k_A);

  Serial.print("w = ");
  Serial.println(gtzl_w_A);

  Serial.print("cosine = ");
  Serial.println(gtzl_cosine_A);

  Serial.print("coeff = ");
  Serial.println(gtzl_coeff_A);

  Serial.println();

  Serial.print("Target frequency B: ");
  Serial.print(target_freq_B);
  Serial.println(" Hz");

  Serial.print("k = ");
  Serial.println(gtzl_k_B);

  Serial.print("w = ");
  Serial.println(gtzl_w_B);

  Serial.print("cosine = ");
  Serial.println(gtzl_cosine_B);

  Serial.print("coeff = ");
  Serial.println(gtzl_coeff_B);
...
void loop() {
  ...
  gtzl_magnitude_A = gtzl_magnitude_B = num_adc_conversions = Q0_A = Q1_A = Q2_A = Q0_B = Q1_B = Q2_B = 0;
  ...
  gtzl_magnitude_A = sqrt(sq(Q1_A) + sq(Q2_A) - Q1_A * Q2_A * gtzl_coeff_A);
  gtzl_magnitude_B = sqrt(sq(Q1_B) + sq(Q2_B) - Q1_B * Q2_B * gtzl_coeff_B);
  Serial.print(gtzl_magnitude_A);
  Serial.print(",");
  Serial.print(gtzl_magnitude_B);
  Serial.println();
...
ISR (ADC_vect) {
  ...
  Q0_A = gtzl_coeff_A * Q1_A - Q2_A + ((int16_t)ADCW - 512);
  Q2_A = Q1_A;
  Q1_A = Q0_A;
  Q0_B = gtzl_coeff_B * Q1_B - Q2_B + ((int16_t)ADCW - 512);
  Q2_B = Q1_B;
  Q1_B = Q0_B;
  ++num_adc_conversions;
